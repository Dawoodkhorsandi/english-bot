...
insets. A dependency-free CSS-grid heatmap renders ~4 months of activity
(no chart library). UI conventions (theme
...
| `/api/feed?limit&cursor` | GET | Newest-first, cursor-paginated page of pooled posts for the app's social feed (`FeedPage`): all kinds at the user's level (plus `tip` at the default level), ordered by `content_pool.id` DESC; `cursor` = the previous page's `next_cursor` (0/absent = first page). HTML preserved, **read-only** (never records). Returns `{items:[{id,kind,term,meaning,text,ts}], next_cursor, has_more}`. Rate-limited. |
| `/api/feed/next?kind?` | GET | Next chat-feed message for the in-app "Chat" tab (`FeedNext`): the kind the broadcast scheduler would send for the current wall-clock slot (drill/word alternation via `feedKindForSlot`), or an explicit `?kind=` override (`feedKinds`: drill/word/idiom/collocation/story/tip). Pool-only (unseen → recycled → oldest), **HTML preserved** for rich rendering, and **does not record** (read-only mirror of the pool). Rate-limited. Returns `{available, kind, term, meaning, text, ts}`. |
| `/api/lookup?term` | GET | Free-text word lookup (the app's chat input), mirroring the bot's plain-text lookup (`lookupWordCard`): generates a `/word`-style card at the user's level via the AI chain, pools and records it. Rejects sentence-length input. Rate-limited. Returns `{available, term, text, meaning}` or `{available:false, message}`. |
