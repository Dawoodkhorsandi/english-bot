...
| Profile drill-down | `#board-profile` via `showBoardSub`; `.profile-head` + `.avatar-fallback.lg` + `.kudos-btn`(`.on`); compare rows `.cmp-row`/`.cmp-track`/`.cmp-fill.me`/`.cmp-fill.them`/`.cmp-up`/`.cmp-down`/`.cmp-tie`; reuses `heatmapHTML` | Tap a leaderboard row → head-to-head stat comparison + their heatmap + 👏 kudos toggle. Addressed by opaque `id`, never `chat_id` (✅ v1.32.0) |
